...
export function AttendanceScreen() {
  ...
  const onTimePct = totalPresent > 0
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        <MonthSelector label={monthLabel} onPress={() => setPickerVisible(true)} />
        <MonthYearPickerModal
          visible={pickerVisible}
          onClose={() => setPickerVisible(false)}
          onSelect={(m, y) => { setMonthIndex(m); setYear(y); }}
        ...
        <AttendanceRecordDetailModal
          ...
          onClose={() => { setDetailModalVisible(false); setSelectedRecord(null); }}
          record={selectedRecord}
        />

        <View style={styles.summaryGrid}>
          <View style={[styles.summaryCard, styles.summaryCardPrimary]}>
            <CalendarCheck size={20} color={COLORS.textPrimary} style={styles.summaryIcon} />
            <Text style={[styles.summaryValue, styles.summaryValueOnPrimary]}>{totalPresent}</Text>
            <Text style={[styles.summaryLabel, styles.summaryLabelOnPrimary]}>Present</Text>
          </View>
          <View style={styles.summaryCard}>
            <Clock size={20} color={COLORS.primary} style={styles.summaryIcon} />
            <Text style={styles.summaryValue}>{totalHours.toFixed(1)}</Text>
            <Text style={styles.summaryLabel}>Total hrs</Text>
          </View>
          <View style={styles.summaryCard}>
            <Clock size={20} color={COLORS.textSecondary} style={styles.summaryIcon} />
            <Text style={styles.summaryValue}>{avgHours.toFixed(1)}h</Text>
            <Text style={styles.summaryLabel}>Avg/day</Text>
          </View>
          <View style={styles.summaryCard}>
            <AlertCircle size={20} color={COLORS.warning} style={styles.summaryIcon} />
            <Text style={styles.summaryValue}>{daysLate}</Text>
            <Text style={styles.summaryLabel}>Late</Text>
          </View>
          <View style={styles.summaryCard}>
            <XCircle size={20} color={COLORS.error} style={styles.summaryIcon} />
            <Text style={styles.summaryValue}>{daysAbsent}</Text>
            <Text style={styles.summaryLabel}>Absent</Text>
          </View>
          <View style={styles.summaryCard}>
            <Text style={[styles.summaryValue, styles.summaryValueSmall]}>{onTimePct}%</Text>
        ...
        <View style={styles.sectionHeader}>
          ...
          <Text style={styles.sectionSub}>
            {filtered.length} record{filtered.length !== 1 ? 's' : ''} · tap for details
          </Text>
        </View>
        ...
        {filtered.length === 0 ? (
          <View style={styles.emptyLogs}>
            <CalendarCheck size={48} color={COLORS.textTertiary} style={styles.emptyIcon} />
            <Text style={styles.emptyTitle}>No records this month</Text>
            <Text style={styles.emptyText}>Attendance for {monthLabel} will appear here after you punch in/out.</Text>
          </View>
        ) : (
          filtered.map((r) => (
            <DailyLogCard
...
const styles = StyleSheet.create({
  ...
  summaryCardPrimary: { backgroundColor: COLORS.primary, borderColor: COLORS.primary },
  summaryLabelOnPrimary: { color: 'rgba(255,255,255,0.9)' },
  summaryValueOnPrimary: { color: COLORS.textPrimary },
  summaryIcon: { marginBottom: THEME.spacing.xs },
  summaryLabel: { fontSize: 10, fontWeight: '600', color: COLORS.textTertiary, letterSpacing: 0.3, marginTop: 2 },
  ...
  logHeaderWorked: { flex: 1, fontSize: 10, fontWeight: '700', color: COLORS.textTertiary, letterSpacing: 0.5 },
  logHeaderStatus: { fontSize: 10, fontWeight: '700', color: COLORS.textTertiary, letterSpacing: 0.5 },
  emptyLogs: { paddingVertical: THEME.spacing.xxl, alignItems: 'center' },
  emptyIcon: { marginBottom: THEME.spacing.md, opacity: 0.6 },
  emptyTitle: { fontSize: 16, fontWeight: '600', color: COLORS.textPrimary, marginBottom: THEME.spacing.sm },
  emptyText: { fontSize: 13, color: COLORS.textSecondary, textAlign: 'center', paddingHorizontal: THEME.spacing.lg },
});
