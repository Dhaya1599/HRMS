...
function authReducer(state: AuthState, action: AuthAction): AuthState {
  switch (action.type) {
    ...
    case 'LOGIN_SUCCESS':
      return { user: action.payload.user, isAuthenticated: true, isLoading: false, error: null };
    case 'LOGIN_ERROR':
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const bootstrapAsync = async () => {
      try {
        ...
        if (token && userData) {
          dispatch({ type: 'RESTORE_TOKEN', payload: { user: userData, token } });
        } else {
  ...
  const refreshUser = async () => {
    const userData = await appStorage.get<User>(STORAGE_KEYS.USER_DATA);
    if (userData) dispatch({ type: 'SET_USER', payload: userData });
  };

  const value: AuthContextType = { state, login, loginDemo, logout, refreshUser };
