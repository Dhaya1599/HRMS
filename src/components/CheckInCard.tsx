...
const getGeofenceColor = (status: string): string => {
  switch (status) {
    ...
    case 'outside': return COLORS.warning;
    ...
    <Card elevation="lg" style={styles.container}>
      ...
      {geofenceStatus === 'outside' && (
        <View style={styles.warningContainer}>
          ...
          <Text style={styles.warningText}>Move closer to the office to check in</Text>
        </View>
      )}
      ...
      <View style={styles.buttonContainer}>
        {!hasCheckedIn ? (
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Text style={styles.buttonText}>{loading ? 'Punching In...' : 'Punch In'}</Text>
          </TouchableOpacity>
        ) : !checkOutTime ? (
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Text style={styles.buttonText}>{loading ? 'Punching Out...' : 'Punch Out'}</Text>
          </TouchableOpacity>
        ) : (
...
const styles = StyleSheet.create({
  container: { marginHorizontal: THEME.spacing.lg, marginVertical: THEME.spacing.md },
  statusRow: {
    flexDirection: 'row',
  ...
  statusText: { flex: 1 },
  statusLabel: { ...THEME.typography.caption, color: COLORS.textSecondary },
  statusValue: { ...THEME.typography.body, color: COLORS.textPrimary, fontWeight: '600', marginTop: THEME.spacing.xs },
  distance: { ...THEME.typography.bodySmall, color: COLORS.primary, fontWeight: '600' },
  warningContainer: {
    flexDirection: 'row',
  ...
  warningText: { ...THEME.typography.caption, color: COLORS.warning, fontWeight: '500', flex: 1 },
  timeSection: { marginVertical: THEME.spacing.lg },
  timeRow: { flexDirection: 'row', alignItems: 'center', marginBottom: THEME.spacing.md, gap: THEME.spacing.md },
  timeLabel: { ...THEME.typography.bodySmall, color: COLORS.textSecondary, flex: 1 },
  timeValue: { ...THEME.typography.body, color: COLORS.textPrimary, fontWeight: '600' },
  buttonContainer: { marginVertical: THEME.spacing.lg },
  button: {
  ...
  buttonText: { ...THEME.typography.body, color: COLORS.background, fontWeight: '700' },
